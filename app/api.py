...
def create_app() -> FastAPI:
    ...
    # ── change the ChatGPT account from the UI (paste a fresh cookie, no .env / redeploy) ──
    @app.post("/admin/account")
    async def admin_account_set(request: Request):
        _require_admin(request)
        _check_csrf(request)
        try:
            body = await request.json()
        ...
        SERVICE.set_account_cookie(cookie)   # re-bootstraps the single browser (cookie not stored)
        return {"ok": True}
